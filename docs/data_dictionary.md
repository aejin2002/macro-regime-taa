...
| `bei_duration_gate_signals.csv` | `run-bei-duration-gate` | `DGS10`, `T10YIE`, `dgs10_change_1m`, `bei_change_1m`, `bei_change_3m`, `tlt_return_1m`, `raw_bei_duration_gate`, `tradable_bei_duration_gate` (see Methodology, "BEI Duration Risk Gate (v1.1)") |
| `bei_duration_gate_allocations.csv` | `run-bei-duration-gate` | `macro_regime`, `base_IEF`/`base_TLT`/`base_BIL`, `duration_pool`, `gate_state`, `final_IEF`/`final_TLT`/`additional_BIL`/`final_BIL`, plus every other asset's final weight |
| `bei_duration_gate_monthly_returns.csv` | `run-bei-duration-gate` | Month-end return per strategy (`v1_0_primary`, `v1_1_bei_duration_gate`, post-cost), index = date |
| `bei_duration_gate_annual_returns.csv` | `run-bei-duration-gate` | Calendar-year return per strategy (post-cost), index = year |
| `bei_duration_gate_summary.csv` | `run-bei-duration-gate` | One row per strategy: start/end date, CAGR, annualized vol, Sharpe, Sortino, max drawdown, Calmar, monthly win rate, annual positive-year ratio, avg annual turnover, final value -- pre-cost and post-cost |
| `bei_duration_gate_regime_analysis.csv` | `run-bei-duration-gate` | Long-format table (`analysis_type`, `key`, `series`, `value`): per-regime average monthly return, and a period breakdown (2013/2018/2020/2021/2022/2023-2024) of cumulative return, max drawdown, and gate-ON month count per strategy |
